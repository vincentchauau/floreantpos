	...
	public synchronized void doLogin() {
		try {
                    ...
                    if (TerminalConfig.isDisablePIN())
                        user = UserDAO.getInstance().findUserBySecretKey("1111");
                    else user = PasswordEntryDialog.getUser(Application.getPosWindow(), Messages.getString("LoginView.1"), Messages.getString("LoginView.2")); //$NON-NLS-1$ //$NON-NLS-2$
                    if (user == null) {
