...
public class CustomPaymentReportView extends JPanel {
    private JXDatePicker fromDatePicker = UiUtil.getCurrentMonthStart();
    private JXDatePicker toDatePicker = UiUtil.getCurrentMonthEnd();
    private JButton btnGo = new JButton(com.floreantpos.POSConstants.GO);
    private JPanel reportContainer;
    public CustomPaymentReportView() {
        ...
        btnGo.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    POSMessageDialog.showError(CustomPaymentReportView.this, POSConstants.ERROR_MESSAGE, e1);
                }
            }
        });
    }
    private void viewReport() throws Exception {
        ...
        List<CustomPaymentTransaction> transactions = (List<CustomPaymentTransaction>) PosTransactionDAO.getInstance().findTransactions(
                CustomPaymentTransaction.class, fromDate, toDate);
        int saleCount = 0;
        double totalSales = 0;
        double totalTips = 0;
        for (CustomPaymentTransaction transaction : transactions) {
            ++saleCount;
        ...
        map.put("reportTitle", POSConstants.CUSTOM_PAYMENT_REPORT.toUpperCase()); //$NON-NLS-1$ //$NON-NLS-2$
        map.put("fromDate", getDateString(fromDate)); //$NON-NLS-1$
        map.put("toDate", getDateString(toDate)); //$NON-NLS-1$
        map.put("reportTime", getDateString(currentTime)); //$NON-NLS-1$
